...
class Crawler(object):
    ...
    def __init__(self, root, depth_limit, path, confine=None, exclude=[], locked=True, filter_seen=True):
        self.root = root
        ...
        self.path = path                # the path provided by user
        ...
        while not q.empty():
            ...
            if [] == do_not_follow:
                try:
                    ...
                    self.num_followed += 1
                    page = Fetcher(this_url, self.path)
                    page.fetch()
    ...
    def getArticles(self, soup):
        ...
        if len(articles) == 0:
            ...
            for article in articles:
                try:
                    # filepath = self.path + 'DD' + str(uuid.uuid4()) + '.txt'
                    # file=codecs.open(filepath, mode='w',encoding='UTF-8')
                    art_str = []
                    header =article.header
                    if header is not None:
                        for title in header.stripped_strings:
                            art_str.append(" "+title+" ")
                            #print(title)
                            # file.write(title)
                    ...
                    if paras is not None:
                        for para in paras:
                            #print(para.get_text())
                            for paraString in para.stripped_strings:
                                art_str.append(" "+paraString+" ")
                                # print(paraString)
                                # file.write(paraString)
                        # print(div.get_text())
                    if len(art_str) > 20:
                        filepath = self.path + 'DD' + str(uuid.uuid4()) + '.txt'
                        file = codecs.open(filepath, mode='w',encoding='UTF-8')
                        file.writelines(art_str)
                        file.close()
        ...
        #self._addHeaders(request)
            ...
            try:
                ...
                if mime_type != "text/html":
                    raise OpaqueDataException("Not interested in files of type %s" % mime_type, mime_type, self.url)
                #content = unicode(data.read(), "utf-8", errors="replace")
                content = unicode(r.text)
                soup = BeautifulSoup(content, 'html.parser')
                tags = soup('a')
                self.getArticles(soup)
...
def getLinks(url, path):
    page = Fetcher(url, path)
    page.fetch()
...
def main():    
    ...
    crawler = Crawler(url, depth_limit, path, confine_prefix, exclude)
    crawler.crawl()
